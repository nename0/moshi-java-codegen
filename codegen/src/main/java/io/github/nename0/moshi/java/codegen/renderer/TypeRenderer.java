...
public final class TypeRenderer {
    ...
    public CodeBlock render(TypeName typeName) {
        ...
        if (typeName instanceof ArrayTypeName) {
            // Types.arrayOf()
            return CodeBlock.of("$T.arrayOf($L)", Types.class, render(((ArrayTypeName) typeName).componentType));
        }
        if (typeName instanceof ParameterizedTypeName) {
            ...
            if (enclosingClassName != null)
                builder.add("newParameterizedTypeWithOwner($L,$W", render(enclosingClassName));
            else
                builder.add("newParameterizedType(");
        ...
        if (typeName instanceof WildcardTypeName) {
            ...
            return CodeBlock.of("$T.$N($L)", Types.class, method, render(target));
        }
        if (typeName instanceof TypeVariableName) {
            // from second constructor param
            return CodeBlock.of("$N[$L]", typeConstructorParam, typeVarNames.indexOf(((TypeVariableName) typeName).name));
        }
